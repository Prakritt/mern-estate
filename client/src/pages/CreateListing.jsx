    ...
    const handleImageSubmit = (e)=>{
        if(files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            Promise.all(promises).then(urls=>{
                ...
                setImageUploadError(false); // To remove previous error if there are any...
                setUploading(false);
            }).catch(err=>{
    ...
    const storeImage = async(file)=>{
        return new Promise((resolve,reject)=>{
            ...
            uploadTask.on("state_changed",(snapshot)=>{
                ...
                console.log("The upload is " + progress + "% complete");
            },(error)=>{
                reject(error);
            },()=>{
        ...
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            <div className="flex flex-col gap-4 flex-1">
                ...
                <div className="flex flex-wrap gap-6">
                    <div className="flex items-center gap-2">
                        <input onChange={handleChange} value={formData.bedrooms}  type="number" id="bedrooms" min='1' max="10" required className='
                        p-3 border border-gray-300 rounded-lg' />
                        <p>Beds</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <input onChange={handleChange} value={formData.bathrooms} type="number" id="bathrooms" min='1' max="10" required className='
                        p-3 border border-gray-300 rounded-lg' />
                        <p>Baths</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <input onChange={handleChange} value={formData.regularPrice} type="number" id="regularPrice" min='50' max="1000000" required className='
                        p-3 border border-gray-300 rounded-lg' />
                    ...
                    </div>
                    {
                        ...
                        (
                            <div className="flex items-center gap-2">
                                <input onChange={handleChange} value={formData.discountPrice} type="number" id="discountPrice" min='0' max="100000" required className='
                                p-3 border border-gray-300 rounded-lg' />
                                <div className="flex flex-col items-center">
                                    <p> Discount Price</p>
                                    <span className='text-xs'>($/months)</span>
            ...
            <div className="flex flex-col flex-1 gap-4">
                ...
                <div className="flex gap-4">
                    <input onChange={(e)=>setFiles(e.target.files)} type="file" id="images" accept="image/*" multiple className='
                    p-3 border border-gray-300 rounded-lg w-full' />
                ...
                <p className='text-red-700 text-sm'>{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url,index)=>{
                       return (
                        <div key = {url} className='flex justify-between p-3 gap-2 border items-center'>
                            <img src={url} alt="listing image" className='w-20 h-20 object-contain roudned-lg' />
                ...
                <button disabled={loading || uploading} className='p-3 bg-slate-700 text-white rounded-lg uppercase 
                 hover:opacity-95 disabled:opacity-85'>{loading?'Creating...':"Create Listing"}</button>
